...
tunnel-client requires at least one organization or workspace attachment when it creates a tunnel. Open Organization Settings to find the organization context. If your tunnel belongs to a ChatGPT workspace, enter that workspace ID in Bridge Settings. You may enter multiple IDs separated by commas.
...
After the terminals show the tunnels as ready, open ChatGPT Apps/Connectors, create or edit each app, choose the matching tunnel, then scan its MCP tools. Keep the three capabilities separate so one tunnel failure does not remove every local capability at once.
